...
export async function messageRoutes(app: FastifyInstance) {
  ...
  app.get("/api/channels/:channelId/messages", { preHandler: requireAuth as any }, async (req, rep) => {
    ...
    const msgs = await db.message.findMany({
      ...
      include: { author: { select: { id: true, username: true, displayName: true, avatarUrl: true, status: true } }, attachments: true },
    });
