...
import { getSpriteForBuilding } from "../../lib/spriteMap";
          ...
          <div className="grid grid-cols-2 gap-3 mb-6">
            {g.starterBuildings.map((def: any) => (
              <button key={def.id} onClick={() => togglePick(def.id)}
                ...
                <img src={getSpriteForBuilding(def.id, 0)} alt="" className="w-12 h-12 object-contain" />
    ...
    <div className="border-t overflow-y-auto" style={{borderColor:"var(--border)", maxHeight:"180px"}}>
              ...
              return (
                <button key={def.id} onClick={() => city.coins >= cost && queueCount < 2 && onBuild(def.id, selectedSlot.x, selectedSlot.y)}
                  ...
                  <div className="flex items-center gap-1.5 mb-1">
                    <img src={getSpriteForBuilding(def.id, 0)} alt="" className="w-8 h-8 object-contain" />
      ...
      {selectedBuilding && selectedDef && (
        <div className="p-3 fade-in">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-2">
              <img src={getSpriteForBuilding(selectedDef.id, selectedBuilding.level)} alt="" className="w-10 h-10 object-contain" />
              <div>
            ...
            <div className="mt-2 space-y-1">
              {(city.buildQueue as any[]).map((q: any) => {
                ...
                return def ? <div key={q.buildingId} className="flex items-center gap-2 text-xs">
                  <img src={getSpriteForBuilding(def.id, 0)} alt="" className="w-5 h-5 object-contain" /><span style={{color:"var(--text-primary)"}}>{def.name}</span><span style={{color:"var(--warning)"}}>⏱{rem}s</span>
                </div> : null;
              })}
            </div>
          )}
