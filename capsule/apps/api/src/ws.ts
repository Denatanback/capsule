  ...
  app.get("/ws", { websocket: true }, (socket, req) => {
    ...
    socket.on("message", async (raw) => {
      ...
      if (event === "message:send") {
        const { channelId, content, fileIds } = data;
        if (!channelId || (!content?.trim() && (!fileIds || fileIds.length === 0))) return;
      ...
      if (event === "dm:send") {
        const { dmChannelId, content } = data;
        ...
        try {
          ...
          const m = await db.dMMessage.create({
            data: { dmChannelId, authorId: userId, content: content.trim() },
            include: { author: { select: { id: true, username: true, displayName: true, avatarUrl: true } } },
          });
