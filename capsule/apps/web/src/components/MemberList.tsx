...
export default function MemberList() {
  ...
  const onM = members.filter((m) => onl.includes(m.userId));
  const offM = members.filter((m) => !onl.includes(m.userId));
  ...
  const kick = async (uid) => {
    ...
    try { await api.kickMember(sd.id, uid); refresh(); } catch (e) { alert(e.message); }
  };
  const ban = async (uid) => {
    if (!sd) return;
    const reason = prompt("Ban reason (optional):");
    if (reason === null) return;
    try { await api.banMember(sd.id, uid, reason || undefined); refresh(); } catch (e) { alert(e.message); }
  };
  const promote = async (uid) => {
    if (!sd || !confirm("Promote to Admin?")) return;
    try { await api.promoteMember(sd.id, uid); refresh(); } catch (e) { alert(e.message); }
  };
  const demote = async (uid) => {
    if (!sd || !confirm("Demote to Member?")) return;
    try { await api.demoteMember(sd.id, uid); refresh(); } catch (e) { alert(e.message); }
  };
  const transfer = async (uid) => {
    if (!sd || !confirm("Transfer ownership? You will become Admin.")) return;
    try { await api.transferOwnership(sd.id, uid); refresh(); } catch (e) { alert(e.message); }
  };
    ...
    return (
      <div className="flex items-center gap-2 px-2 py-1 rounded hover:bg-gray-100 dark:hover:bg-gray-800 relative group"
        ...
        <div className="min-w-0 flex-1">
          <p className={"text-xs truncate " + (g ? "" : "text-gray-400")}>{m.user?.displayName}</p>
  ...
  return (
    <div className="w-52 border-l border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 overflow-y-auto p-2 shrink-0">
      {onM.length > 0 && <>
        <p className="text-[10px] uppercase text-gray-400 font-semibold px-2 mb-1">Online - {onM.length}</p>
        {onM.map((m) => <Row key={m.id} m={m} g={true} />)}
      </>}
      {offM.length > 0 && <>
        <p className="text-[10px] uppercase text-gray-400 font-semibold px-2 mb-1 mt-3">Offline - {offM.length}</p>
        {offM.map((m) => <Row key={m.id} m={m} g={false} />)}
      </>}
    </div>
  );
}
